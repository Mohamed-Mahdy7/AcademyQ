const DangerCard = ({name, warning, danger, button, info, onContact}) => {
    return (
        <div className="card-danger my-3">
            <div className="flex items-start w-full">
                <section className="stat-icon-wrap-danger">
                    <svg className="w-6 h-6 text-danger fill-current" viewBox="0 0 56 56" xmlns="http://www.w3.org/2000/svg">
                ...
                <section className="flex-1">
                    <div className="flex justify-between">
                        <p className="heading-3 mt-1">{name}</p>
                        {/* <button className="btn-secondary" onClick={onContact}>{button}</button> */}
                    </div>
                    <span className="badge-warning">{warning}</span>
                    {danger ? <span className="badge-danger-dark">{danger}</span> : null}
