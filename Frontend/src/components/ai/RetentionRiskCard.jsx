  ...
  return (
    <div className="kpi-card">
      <div className="flex items-center justify-between mb-2">
        <p className="kpi-label">{t("retention_risk")}</p>
      ...
      {loading ? (
        ...
        <>
          ...
          <p className="text-caption mb-3">{t("open_alerts")}</p>

          <div className="flex items-center gap-2 mb-4 flex-wrap">
            {stats?.high_risk_count > 0 && (
              <span className="badge-danger">
                {t("high_risk", { count: stats.high_risk_count })}
              </span>
            )}
            {stats?.medium_risk_count > 0 && (
              <span className="badge-warning">
                {t("medium_risk", { count: stats.medium_risk_count })}
              </span>
            )}
            {!stats?.high_risk_count && !stats?.medium_risk_count && (
              <span className="badge-success">{t("all_clear")}</span>
            )}
          </div>
